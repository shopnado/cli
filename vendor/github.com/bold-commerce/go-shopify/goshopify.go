...
func (c *Client) logRequest(req *http.Request) {
	...
	if req.Body != nil {
		...
		teeReq := io.TeeReader(req.Body, &reqBuf)
		request, _ := ioutil.ReadAll(req.Body)
		if len(request) > 0 {
			c.log.Debugf("SENT: %s", string(request))
		}
		req.Body = ioutil.NopCloser(teeReq)
	}
}
...
func (c *Client) logResponse(res *http.Response) {
	...
	if res.Body != nil {
		...
		teeRes := io.TeeReader(res.Body, &resBuf)
		response, _ := ioutil.ReadAll(res.Body)
		if len(response) > 0 {
			c.log.Debugf("RESP: %s", string(response))
		}
		res.Body = ioutil.NopCloser(teeRes)
	}
}
